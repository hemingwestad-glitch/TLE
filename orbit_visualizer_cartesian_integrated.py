...
2) Cartesian J2000 state vectors (position + velocity)

Usage examples:
    python orbit_visualizer_cartesian_integrated.py orbital_elements_2503.txt --plot-orbits 6 --plot-step-s 15
    python tle_visualizer.py orbital_elements_2503.txt --plot-orbits 1 --plot-step-s 15 --station-lat 69.29 --station-lon 16.02
    python tle_visualizer.py orbital_state_cartesian.txt --plot-orbits 3 --pass-orbits 16
"""
...
def gmst_rad(d: dt.datetime) -> float:
    ...
    gmst_deg = (280.46061837 +
                ...
                0.000387933 * T * T -
                (T * T * T) / 38710000.0)
    return math.radians(normalize_angle_deg(gmst_deg))
...
def split_on_dateline(lons: np.ndarray, lats: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
class OrbitalElements:
    ...
    def ground_track(self, duration_s: Optional[float] = None,
        ...
        """Compute ground track. Returns (times_s, lats_deg, lons_deg)."""
    ...
    def find_passes(self, station: GroundStation,
        ...
        while i < len(times):
            ...
            if seg_start == 0:
                ...
                rise_s = self._refine_crossing(times[seg_start - 1], times[seg_start], station, min_elev_deg)

            if seg_end == len(times) - 1:
                set_s = times[-1]
            else:
                set_s = self._refine_crossing(times[seg_end], times[seg_end + 1], station, min_elev_deg)
            ...
            passes.append(VisibilityPass(
                rise_utc=self.epoch + dt.timedelta(seconds=float(rise_s)),
                set_utc=self.epoch + dt.timedelta(seconds=float(set_s)),
                tca_utc=self.epoch + dt.timedelta(seconds=float(tca_s)),
                max_elevation_deg=float(max_el),
                slant_range_km=float(slant)
            ))
            i = seg_end + 1
        return passes
...
class CartesianState:
    """Propagates from J2000 Cartesian state via RK4 numerical integration."""
    epoch: dt.datetime
    r0_km: np.ndarray
    v0_km_s: np.ndarray
    period_s: float
    ...
    def _eom(self, rv: np.ndarray) -> np.ndarray:
        """Two-body equations of motion. rv = [x,y,z,vx,vy,vz]."""
        r = rv[:3]
        r_norm = np.linalg.norm(r)
        a = -MU_EARTH_KM3_S2 / r_norm**3 * r
        return np.concatenate([rv[3:], a])

    def _rk4_step(self, rv: np.ndarray, dt_s: float) -> np.ndarray:
        """Single RK4 step."""
        k1 = self._eom(rv)
        k2 = self._eom(rv + 0.5 * dt_s * k1)
        k3 = self._eom(rv + 0.5 * dt_s * k2)
        k4 = self._eom(rv + dt_s * k3)
        return rv + (dt_s / 6.0) * (k1 + 2 * k2 + 2 * k3 + k4)

    def propagate_eci(self, times_s: np.ndarray) -> np.ndarray:
        """
        Propagate in ECI (km) at given times (seconds from epoch).
        Returns array shape (N, 3).
        """
        times_s = np.asarray(times_s, dtype=float)
        rv = np.concatenate([self.r0_km.astype(float), self.v0_km_s.astype(float)])
        results = np.empty((len(times_s), 3), dtype=float)
        t_prev = 0.0
        for i, t in enumerate(times_s):
            dt_step = t - t_prev
            if abs(dt_step) > 1e-12:
                n_sub = max(1, int(abs(dt_step) / 30.0))
                sub_dt = dt_step / n_sub
                for _ in range(n_sub):
                    rv = self._rk4_step(rv, sub_dt)
            results[i] = rv[:3]
            t_prev = t
        return results

    def propagate_ecef(self, times_s: np.ndarray) -> np.ndarray:
        """Propagate in ECEF (km) at given times (seconds from epoch)."""
        r_eci = self.propagate_eci(times_s)
        r_ecef = np.empty_like(r_eci)
        for i, t in enumerate(times_s):
            theta = gmst_rad(self.epoch + dt.timedelta(seconds=float(t)))
...
SatelliteState = Union[OrbitalElements, CartesianState]


# ============================================================
# PARSING AV ELEMENTFIL / KARTESISKE FILER
# ============================================================
FIELDS_ORBITAL = {
    "Spacecraft": ("name", str),
...
FIELDS_CARTESIAN = {
    "Spacecraft": ("name", str),
    "UTC time at deployment": ("epoch", parse_utc_datetime),
    "J2000 (x,y,z) Position (km)": (
        "r0_km",
        lambda s: np.array([float(x) for x in s.strip().strip("[]").split()], dtype=float),
    ),
    "J2000 (x,y,z) Velocity (km/s)": (
        "v0_km_s",
        lambda s: np.array([float(x) for x in s.strip().strip("[]").split()], dtype=float),
    ),
    "Period (s)": ("period_s", float),
...
def _parse_fields(file_obj, field_map: dict) -> dict:
    result = {}
    for line in file_obj:
        ...
        if key not in field_map:
            continue
        field_name, parser = field_map[key]
        result[field_name] = parser(value)
    return result


def parse_file(file_obj) -> SatelliteState:
    """Parse either orbital elements or Cartesian state from text file."""
    # Read once so we can inspect format.
    ...
    # Cartesian format if it contains explicit J2000 vectors.
    if any(k.startswith("J2000 (x,y,z) Position") or k.startswith("J2000 (x,y,z) Velocity") for k in keys):
        result = _parse_fields(lines, FIELDS_CARTESIAN)
        required = {"epoch", "r0_km", "v0_km_s", "period_s"}
        missing = required - set(result.keys())
        if missing:
            raise ValueError(f"Missing fields: {sorted(missing)}")
        if result["period_s"] <= 0:
            raise ValueError("Period must be positive")
        return CartesianState(**result)

    # Fallback to classical orbital elements.
    result = _parse_fields(lines, FIELDS_ORBITAL)
    required = {
        "epoch", "semi_major_axis_km", "eccentricity", "inclination_deg",
...
# ============================================================
# HJELPEFUNKSJONER FOR SANITY CHECK
# ============================================================
def sanity_check_cartesian(state: CartesianState) -> None:
    """Small sanity check for Cartesian input."""
    r_mag = np.linalg.norm(state.r0_km)
    print(f"|r0| = {r_mag:.3f} km")
    r_ecef_t0 = state.propagate_ecef(np.array([0.0]))[0]
    print(f"ECEF beregnet t=0: {r_ecef_t0}")
...
def main():
    ...
    parser.add_argument("--station-lat", type=float, default=63.4305,
    ...
    parser.add_argument("--station-alt", type=float, default=10.0,
    ...
    if args.name_override and hasattr(elements, "name"):
    ...
    if isinstance(elements, OrbitalElements):
        ...
        print(f"J2000 position (km): {elements.r0_km}")
        print(f"J2000 velocity (km/s): {elements.v0_km_s}")
        sanity_check_cartesian(elements)
